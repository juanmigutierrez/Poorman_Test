...
# 2) Using `stats::reshape()`, change the following dataset to wide format such that there is one row per sample and then 
...
# 3) Using `stats::reshape()`, reshape the following data to long format such that there are five columns: 
# `sex`, `age`, `id`, `exam` and `score`. Then reshape the data back into the original format.
...
# 4) Using `stats::reshape()`, reshape the following data to a wide format with the resulting columns: 
# `id`, `min.1`, `max.1`, `min.2`, `max.2.` Then reshape the data back to the original shape.
